...
pub mod raw
{
    ...
    fn raw_opts(raw_args: cli::RawArgs) -> io::Result<()>
    {
        ...
        if raw_args.target.is_file()
        {
            match is_supported_format(&raw_args.target)
            {
                Ok(target) => match target
                {
                    ...
                    {
                        info!(?mime_type);
                        let outpath = raw_args.outpath.unwrap_or(
...
pub mod roast
{
    ...
    pub fn roast_opts(roast_args: cli::RoastArgs) -> io::Result<()>
    {
        let terminfodb = Database::from_env().map_err(|e| {
            error!(err = ?e, "Unable to access terminfo db. This is a bug!");
            io::Error::new(
        ...
        {
            let newworkdir =
                workdir.join(target_path.file_name().unwrap_or(target_path.as_os_str()));
            ...
            Some(ext) =>
            {
                ...
                if is_tar
                {
                    ...
                    match some_ft
                    {
                        ...
                        {
                            let message =
                                format!("Unsupported file type: {}", ext.to_string_lossy());
