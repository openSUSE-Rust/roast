...
) -> io::Result<()>
{
        ...
        && !is_hidden(entry_as_path_canonicalized, hidden, ignore_git, root)
    {
        ...
        if let Some(excluded_path) = exclude
        {
            if !excluded_path.as_os_str().is_empty()
            {
                let is_stripped_genesis_path_parent =
                    genesis_path_parent.strip_prefix(excluded_path);
